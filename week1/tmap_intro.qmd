...
# used 'read stars' for raster data

# country boundaries
world_vector <- read_sf(here("week1","data", "worldvector.gpkg"))
# used 'read sf' for vector data
...
```{r}
# Plotting elevation (a raster dataset)
tm_shape(world_elevation) +
  tm_raster() + # shows elevation as a raster
  tm_shape(world_vector)+
  tm_borders() + #show elevation as a map of the world
  tm_shape(world_cities) +
  tm_dots+ # show cities as dots
  tm_text("name", size = 0.7, col = "purple") # show names of cities
```
...
```{r}
# change the order of your layers to see what it looks like
tm_shape(world_cities)+ 
  tm_dots()+
  tm_text("name")+
  tm_shape(world_elevation)+
...
tm_shape(world_vector)+
  tm_borders()+
tm_shape(world_cities, is.main = TRUE)+ # plotting world cities last, set as main shape, makes everything the same coordinate reference system as cities 
  tm_dots()+
  tm_text("name")
```
...
```{r}
# specify the extent shown in the map (long, long, lat, lat)
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + #add bounding box to the extent of the amp
  tm_raster()+
tm_shape(world_vector)+
  tm_borders()
```
...
```{r}
# change the extent to be the same as world cities
...
```{r}
# map of the world with fill and dashed border
tm_shape(world_vector) +
  tm_polygons(fill = "lightblue",
              col = "black",
              lwd = 0.5
              lty = "dashed") # fill, border color, width of border, type
...
```{r}
# map of the world with just the borders
tm_shape(world_vector)+
  tm_borders(col = "red")
```
...
tm_shape(world_vector)+
  tm_fill(fill = "CO2_emissions") # CO2 emissions included in world vector dataset
```
...
```{r}
# just the centroid of the cities
tm_shape(world_cities)+
  tm_symbols()
```
...
```{r}
# cities represented as squares
tm_shape(world_cities)+
  tm_symbols(shape = 22) # plots a square shape where the cities are
# can also use tm_squares()

# cities represented as circles
tm_shape(world_cities)+
  tm_bubbles()
```
...
```{r}
# create a gradient for fill with data deom world_vector dataset
tm_shape(world_vector)+
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("yellow","darkgreen")), # lower and upper colors of a gradient
...
# use viridislite package for fill colors
tm_shape(world_vector)+
  tm_polygons( fill = "life_expectancy",
...
```{r}
# create map with city symbol corresponding to city population size
tm_shape(world_vector)+
  tm_polygons(fill = "lightblue")+
tm_shape(world_cities)+
  tm_symbols(
    size = "pop2020", # variable in world cities dataset
    size.legned = tm_legend(orientation = "portrait")
  )
...
```{r}
# map with shapes of countries and grid
tm_shape(world_vector)+
  tm_fill()+
  tm_grid()
```
...
```{r}
# give your map the curvy world grid
tm_shape(world_vector)+
  tm_fill()+
  tm_graticules()
```
...
```{r}
# create a map witha scale bar and compass
tm_shape(world_vector)+
  tm_fill()+
  tm_scalebar()+
...
tm_shape(world_vector)+
  ...
  tm_title(text = "Global Income",
           frame = TRUE) + # puts a box aorund the title
  tm_layout(style = "watercolor",
            bg.color = "grey96")
  
# try making my own layout
tm_shape(world_vector)+
  tm_polygons(fill = "life_expectancy",
              fill.legend = tm_legend(title = "CO2 Emissions by Country")) +
  tm_layout(color_vision_deficiency_sim = ("proton"),
            style = "beaver") +
  tm_crs("+proj+eqearth") # change the coordinate reference system to match your plot
```
...
```{r}
# make all subseqient plots interactive
tmap_mode("view")
...
tmap_save(map1, here("week1", "map_1.png"))
```
